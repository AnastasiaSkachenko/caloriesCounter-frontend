...
const DishForm: React.FC<AddDishProps> = ({onSuccess,onSuccessEdit, onCancel, dishToEdit}) => {
  ...
  const handleSubmit = async () => {
    ...
    const dishID:number = await  setDish({dish:{...dishInfo, name: dishInfo.name.slice(0,1).toUpperCase() + dishInfo.name.slice(1)}})
  ...
  return (
    <div className='d-flex flex-column justify-content-center bg-light p-3'> 
      <h2>{dishToEdit? "Edit dish":"Create a new Dish"}</h2>
      <label>
        Dish Name:
