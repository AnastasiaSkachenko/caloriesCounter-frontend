...
const MediaScroller: React.FC<MediaScrollerProps> = ({ media, productName, className, width, height }) => {
  ...
  return (
    <div
      id={`carousel-${productName.replace(" ", "-")}`}
    ...
    >
      {media && media.length > 1 && (
        <>
          <button
            ...
            style={{height:35}}
            type="button"
            data-bs-target={`#carousel-${productName.replace(" ", "-")}`}
            data-bs-slide="prev"
          >
      ...
      <div className="carousel-inner mt-3" style={{ width, height }}>
        {media?.map((file, index) => {
          ...
          return (
            <div key={index} className={`carousel-item bg-white  ${index === 0 ? "active" : ""}`} style={{ width, height }}>
              <div
